...
export default function LogDetail() {
  ...
  async function load() {
    ...
    setSession(s)
    setPageTitle(
      s ? `${s.trained_at} · ${s.letter}` : 'Training',
      s?.plan_name || ''
    )
    const grouped = {}
    ;(sets || []).forEach(set => {
      ...
      {session?.duration_seconds && (
        <div className="card" style={{ cursor: 'default', display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
          ...
          <span style={{ fontSize: 14 }}>
            Trainingsdauer: <strong>{formatDuration(session.duration_seconds)}</strong>
          </span>
        </div>
      )}
      ...
      {Object.entries(byExercise).map(([name, sets]) => (
        <div key={name} className="session-card" style={{ cursor: 'default' }}>
          ...
          <table className="sets-table">
            <thead>
              <tr>
                <th>Satz</th><th>KG/S</th><th>KG ges.</th><th>Ziel</th><th>Ist</th><th>✓</th>
              </tr>
            </thead>
            <tbody>
              {sets.map(set => (
...
function IconClock() {
  return <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--accent)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0 }}><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
}
