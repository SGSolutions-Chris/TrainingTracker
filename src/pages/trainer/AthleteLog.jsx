import { useEffect, useState } from 'react'
import { useParams, useNavigate, useLocation } from 'react-router-dom'
import { usePageTitle } from '../../contexts/PageTitleContext'
import { getAthleteAllSessions } from '../../lib/db'
import { formatDate, formatDuration, unitBadgeClass } from '../../lib/utils'

export default function AthleteLog() {
  const { athleteId } = useParams()
  const { state } = useLocation()
  const navigate = useNavigate()
  const { setPageTitle } = usePageTitle()

  const athleteName = state?.name || 'Athlete'
  const [sessions, setSessions] = useState([])
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    setPageTitle(athleteName, 'Trainingsverlauf')
    load()
  }, [athleteId])

  async function load() {
    setLoading(true)
    const { data } = await getAthleteAllSessions(athleteId)
    setSessions(data || [])
    setLoading(false)
  }

  const LETTERS = ['A','B','C','D','E','F','G','H']

  if (loading) return <div className="spinner-wrap" style={{ minHeight: 200 }}><div className="spinner" /></div>
  if (!sessions.length) return <div className="empty"><div className="empty-icon">📅</div><div className="empty-title">Keine Sessions</div></div>

  let lastWeek = ''

  return (
    <div style={{ padding: '4px 16px 16px' }}>
      <p className="section-label">Trainingsverlauf</p>
      {sessions.map(s => {
        const wk = `KW ${s.week_number} · ${s.trained_at.slice(0, 4)}`
        const showWeek = wk !== lastWeek
        lastWeek = wk
        const bc = unitBadgeClass(Math.max(0, LETTERS.indexOf(s.letter)))
        const dur = s.duration_seconds ? ` · ${formatDuration(s.duration_seconds)}` : ''
        return (
          <div key={s.id}>
            {showWeek && <div className="week-head">{wk}</div>}
            <div
              className="log-item"
              onClick={() => navigate(`/trainer/athletes/${athleteId}/log/${s.id}`, { state: { session: s, name: athleteName } })}
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <span className={`badge ${bc}`}>{s.letter}</span>
                <span style={{ fontSize: 14, fontWeight: 500 }}>{s.plan_name}</span>
              </div>
              <div className="log-meta">{formatDate(s.trained_at)}{dur}</div>
            </div>
          </div>
        )
      })}
    </div>
  )
}
